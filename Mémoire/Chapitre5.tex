...
pris des salles du \keyword{centre culturel universitaire (CCU) des sciences techniques},
et l'autre ensemble a été pris au niveau du \keyword{département d'informatique} de
notre université.

L'ensemble de données du CCU contient 985 instances, où 788 instances sont
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{Info-images}
...
\lstinputlisting[firstline=315,lastline=326,
...
de \keyword{l'informatique en nuages} (cloud computing). Nous avons choisi cette plateforme spécialement vu la facilité
...
de \keyword{GNU/Linux} est préinstallé : l'un contient \keyword{Debian 8} et l'autre
contient \keyword{CentOS 7}.

Ayant les mêmes caractéristiques, nous avons remarqué que notre programme s'exécute
plus stablement sur la machine qui \keyword{Debian 8}
comme système d'exploitation que celle qui a \keyword{CentOS 7}. Ce jugement a été fait à
...
sous-ensembles dites \keyword{batches} qui ont tous la même taille\footnote{
