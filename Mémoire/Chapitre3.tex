\setcounter{chapter}{2} % Forcer la numérotation pour les compilations à un seul chapitre
...
Étant donnée que les performances du microcontrôleur sont très limitées, nous
...
\caption{Le schéma des connexions entre les modules du robots}
\end{center}
\end{figure}
...
exemple : la température), il sera envoyé au téléphone comme une chaîne de
...
ou irrigulière, alors l'ultrason envoyé sera perdu et la distance ne peut pas être
...
Concernant la commande de changement de la puissance, elle est toujours exécutée
à l'ordre de l'utilisateur seulement. Elle contient la valeur de la puissance qui
est un nombre entier définit sur l'intervalle $[0-255]$. Cet valeur est initialisée
par défaut à la valeur $127$.
...
se faire dans les deux sens et sont soit périodiques ou apériodiques selon l'action.
...
\begin{itemize}
  \item l'envoi de commandes de mouvement issues par l'utilisateur,
...
sauvegardées sur la mémoire permanente.
...
de direction (avant, arrière, droite et gauche) ou le signal de la
...
du mode de la navigation, c'est-à-dire qu'elle peut être activée sans que la
navigation soit automatique et vice versa.

Les commandes de mouvement et de sauvegarde peuvent être activées soit par les
boutons existant sur l'interface ou par des touches de clavier. Les instructions
permettant de guider la machine sont clairement visibles sur l'interface.
...
\section{La collection et le traitement des données}
...
déplacement de $80$ pixels sur chaque marge, ce qui résulte à une image carrée
de $480 \times 480$ pixels. La taille de l'image est par la suite réduit à
...
nom un préfixe (\texttt{IMG\_}) suivi par un identificateur numérique généré du
temps de la capture (l'\keyword{époque}\footnote{Il représente le nombre de
millisecondes passés depuis le premier Janvier 1970 à minuit dans le temps
universel coordonné (UTC).}). En même temps, dans un autre fichier, les distances
correspondantes à chaque image sont enregistrées avec l'identificateur de cette
dernière, chaque instance étant écrite sur une ligne.
...
est nécessaire pour pouvoir effectuer cette tâche. Cette nécessité a nous mis devant
...
utilisant nos propres outils que nous devons les développer nous-mêmes.

Dans ce travail, nous avons choisi la deuxième méthode. De ce fait, nous avons
fabriqué le robot que nous avons décrit sa construction matérielle et sa
programmation sur le niveau logiciel. Il nous a permet de collecter les images
prises au niveau de sa vision et les distances des obstacles éventuels dans
chaque image. Dans le chapitre suivant, nous allons exploiter ces données en
appliquant un apprentissage automatique supervisé en utilisant la méthode de
